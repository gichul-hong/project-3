...
----
    ...
    --images   해당 컨셉의 생성 이미지 10장이 든 폴더 (컨셉 하나만 지정)

i번째 생성 이미지는 i번째 프롬프트로 만든 것으로 간주하여 짝지어 채점합니다.
파일명 정렬 순서가 프롬프트 줄 순서와 같아야 합니다 (예: 0.png ... 9.png).
"""
...
with torch.no_grad():
    ...
    text_embedding = model.get_text_features(
        **processor(text=prompts, return_tensors="pt", padding=True).to(device)).pooler_output
    image_ref = model.get_image_features(
        **processor(images=reference, return_tensors="pt").to(device)).pooler_output
    image_gen = model.get_image_features(
        **processor(images=images, return_tensors="pt").to(device)).pooler_output
